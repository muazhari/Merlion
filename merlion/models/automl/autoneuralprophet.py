...
#
"""
Automatic hyperparameter selection for NeuralProphet.
"""
...
from merlion.models.forecast.neuralprophet import NeuralProphet
from merlion.utils import TimeSeries

logger = logging.getLogger(__name__)


class AutoNeuralProphetConfig(SeasonalityConfig, ICConfig):
    """
    Config class for `NeuralProphet` with automatic seasonality detection & other hyperparameter selection.
    """

    def __init__(
        self,
        model: Union[NeuralProphet, dict] = None,
        ...
        **kwargs,
        ...
        model = dict(name="NeuralProphet") if model is None else model
        super().__init__(
            model=model,
    ...
    def multi_seasonality(self):
        """
        :return: ``True`` because NeuralProphet supports multiple seasonality.
        """
        return True


class AutoNeuralProphet(ICAutoMLForecaster, SeasonalityLayer):
    """
    `NeuralProphet` with automatic seasonality detection. Automatically detects and adds
    additional seasonalities that the existing NeuralProphet may not detect (e.g. hourly).
    Also automatically chooses other hyperparameters.
    """

    config_class = AutoNeuralProphetConfig
    ...
    def _model_name(self, theta) -> str:
        return f"NeuralProphet({','.join(f'{k}={v}' for k, v in theta.items())})"
